...
```{r, echo = TRUE, eval = FALSE}
download.aia.2014 <- function(target.dir = "/NASA/AIA2014/",
...
```{r, echo = TRUE, eval = FALSE}
aia.index.allchannels <- function(target.dir = "/NASA/AIA2014/"){
  lf <- list.files(target.dir)
  ...
  write.csv(AIAindex.allchannels.POSIX,
            paste0("AIA_index_allChannels_", Sys.Date(),".csv"),
            row.names = F)
}
```
...
```{r, echo = TRUE, eval = FALSE}
lf <- list.files("/NASA/AIA2014/")
all.channel.index.POSIX <- as.POSIXct(read.csv("AIA_index_allChannels.csv",
...
foreach(i = 1:nrow(all.channel.index)) %dopar% {
  ...
  for(j in 1:8){
    temp <- astropy$io$fits$open(paste0("/NASA/AIA2014/AIA", all.channel.index[i, j], ".fits"))
    temp$verify("fix")
  ...
  write_feather(as.data.frame(big.mat), paste0("/NASA/FeatherAIA2014/AIA",
